...
public:
	...
	void rotate( int _angle, int _x, int _y, int _z ){
	...
	void render(){
		...
		glTranslatef(window_width/2 - x, window_height/2 - y, 0);		
...
public:
	...
	void sort_particles(){
		if( particles_len != 0 ){	
			int len = particles_len;
			int max = particles[0].z;
			int max_pos = 0;
...
/*
			while (len > 0){
				for( int i = 0; i < len; i++ ){
					if( particles[i].z > max ){
						max_pos = i;
						max = particles[i].z;
					}
				}

				PARTICLE temp = particles[max_pos];
				particles[ max_pos ] = particles[ len ];
				particles[ len ] = temp;
				particles[ len ].R = 1;
				particles[ len ].G = 1;
				particles[ len ].B = 1;
...
void init_glut( void ){
	...
	glOrtho( 0.0, window_width, window_height, 0.0, 50.0, -50.0 );
//	gluPerspective( 60, window_width/window_height, 0.1, 100 );

/*	GLdouble vvLeft = -1000;
	GLdouble vvRight = 1000;
	GLdouble vvBottom = -1000;
	GLdouble vvTop = 1000;
	GLdouble vvNear = 100;
	GLdouble vvFar = 300;
