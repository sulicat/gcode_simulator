...
//	window.set_fullscreen( true );
	...
	//std::vector<Vector> positions = readF((char * ) "Arch.gcode", 1 );

	PARTICLE * particles = new PARTICLE[10];
	int particles_len = 10;

	for( int  i = 0; i < particles_len; i++ ){
		particles[i] = PARTICLE( 100 + i*25, 100, 0 + i * 2, 20 );
		particles[i].set_color( 0.1 * i, 0, 0.1 * (10 - (i)) );
		render.add( particles[i] );
